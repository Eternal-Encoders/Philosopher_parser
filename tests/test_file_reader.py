...
def test_file_read():
    out_txt, out_imgs = file_reader.read_markdown('__input_TEST__/test.docx', force_reload=True)

    assert len(out_imgs) == 2
    assert isinstance(out_txt, str)

def test_load():
    out_txt, out_imgs = file_reader.read_markdown('__input_TEST__/test.docx', force_reload=False)
    out_txt, out_imgs = file_reader.read_markdown('__input_TEST__/test.docx', force_reload=False)
